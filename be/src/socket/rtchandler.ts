...
type RoomState = {
  ...
  users: Set<string>;         // userId set (optional)
...
export function registerRtcHandlers(io: Server, socket: Socket) {
  ...
  socket.on("room:join", ({ roomId }: { roomId: string }) => {
    ...
    console.log(`[RTC] User ${userId} (socket ${socket.id}) joining room ${roomId}`);
    ...
    // when both present, pick offerer deterministically (lexicographic userId)
    if (state.members.size >= 2 && !state.offererUserId) {
      const users = Array.from(state.users).filter(Boolean).sort();
      state.offererUserId = users[0]; // smaller wins
      
      console.log(`[RTC] Room ${roomId} ready! Offerer: ${state.offererUserId}`);
      console.log(`[RTC] Members: ${Array.from(state.members).join(", ")}`);
      
      // let both know we can start; tell who is offerer
      ...
      console.log(`[RTC] Room ${roomId} waiting for second user (${state.members.size}/2)`);
    }
  });

  // relay offer/answer/candidate to the rest of the room
  socket.on("rtc:offer", (payload: { roomId: string; sdp: RTCSessionDescriptionInit }) => {
    if (!payload?.roomId || !payload?.sdp) {
      console.warn("[RTC] rtc:offer missing roomId or sdp");
      return;
    }
    console.log(`[RTC] Relaying offer from ${socket.id} to room ${payload.roomId}`);
    socket.to(payload.roomId).emit("rtc:offer", payload);
  });

  socket.on("rtc:answer", (payload: { roomId: string; sdp: RTCSessionDescriptionInit }) => {
    if (!payload?.roomId || !payload?.sdp) {
      console.warn("[RTC] rtc:answer missing roomId or sdp");
      return;
    }
    console.log(`[RTC] Relaying answer from ${socket.id} to room ${payload.roomId}`);
    socket.to(payload.roomId).emit("rtc:answer", payload);
  });

  socket.on("rtc:candidate", (payload: { roomId: string; candidate: RTCIceCandidateInit }) => {
